...
create proc [dbo].[q_zu_approval_prl]
(
...
--and q.q_zu_approval_default not like N'1' --zuorn 231123
...
begin
	...
	update q_zu_portal_prl_list
	set
		aktiv=0
	where
		ForetagKod=@c_foretagkod
	and PrisLista=@c_prislista
	and ArtNr=@c_artnr
	and AltEnhetKod=@c_altenhetkod
	and LimitLowAnt=@c_limitlowant
...
begin
	...
	begin
		...
		,	prl.q_zu_approval_approvedby=@c_perssign
		,	prl.q_zu_approval_approveddt=cast(getdate() as date) 
		,	prl.No_Trigger_Exec=1-prl.No_Trigger_Exec
		where 
			prl.ForetagKod=@c_foretagkod 
		and prl.prislista=@c_prislista
