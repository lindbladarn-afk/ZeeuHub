...
go
if not exists(select 1 from sys.objects where name like 'q_zu_CustomerPortal_prl_Add')
begin
	exec('create proc q_zu_CustomerPortal_prl_Add as select 123')
end
go
ALTER PROCEDURE [dbo].[q_zu_CustomerPortal_prl_Add]
	@ForetagKod SMALLINT
,	@c_prislista int
