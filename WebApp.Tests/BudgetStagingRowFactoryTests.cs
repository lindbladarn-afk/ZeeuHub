...
// Verifies the shared mapping from budget rows to Jeeves staging rows.
public sealed class BudgetStagingRowFactoryTests
{
    [Fact]
    public void Create_Maps_Budget_Data_To_Staging_Row()
    {
        ...
        var factory = new BudgetStagingRowFactory();

        var row = factory.Create(new BudgetStagingRowCreateRequest
        {
        ...
        Assert.Contains("\"Account\":\"3000\"", row.RawJson);
