...
{
    public class JeevesPriceUpdateStagingRepository : IPriceUpdateStagingRepository
    {
        private readonly IExcelImportConnectionResolver _connectionResolver;
        public JeevesPriceUpdateStagingRepository(IExcelImportConnectionResolver connectionResolver)
        {
            _connectionResolver = connectionResolver;
        }

        public async Task BulkInsertAsync(IEnumerable<PortalPriceUpdateStagingRow> rows, CancellationToken cancellationToken = default)
        {
            var rowList = rows as IList<PortalPriceUpdateStagingRow> ?? rows.ToList();
            if (rowList.Count == 0)
            {
                return;
            }
            ...
            try
            {
                ...
                {
                    DestinationTableName = "dbo.q_zu_StagingPriceUpdate",
                    BatchSize = 1000,
                    BulkCopyTimeout = 120
                };
        ...
        private static DataTable BuildDataTable(IEnumerable<PortalPriceUpdateStagingRow> rows)
        {
