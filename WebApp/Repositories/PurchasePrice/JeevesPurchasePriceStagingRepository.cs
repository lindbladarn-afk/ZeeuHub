...
{
    public class JeevesPurchasePriceStagingRepository : IPurchasePriceStagingRepository
    {
        private readonly IExcelImportConnectionResolver _connectionResolver;
        public JeevesPurchasePriceStagingRepository(IExcelImportConnectionResolver connectionResolver)
        {
            _connectionResolver = connectionResolver;
        }

        public async Task BulkInsertAsync(IEnumerable<PortalPurchasePriceStagingRow> rows, CancellationToken cancellationToken = default)
        {
            var rowList = rows as IList<PortalPurchasePriceStagingRow> ?? rows.ToList();
            if (rowList.Count == 0)
            {
                return;
            }
            ...
            try
            {
                ...
                {
                    DestinationTableName = "dbo.q_zu_StagingPurchasePrice",
                    BatchSize = 1000,
                    BulkCopyTimeout = 120
                };
        ...
        private static DataTable BuildDataTable(IEnumerable<PortalPurchasePriceStagingRow> rows)
        {
